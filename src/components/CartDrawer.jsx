...
export const CartDrawer = () => {
  const { isCartOpen, setIsCartOpen, cart, removeFromCart, updateCartQuantity, totalCartPrice } = useApp();
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: '400px', maxWidth: '90vw', zIndex: 1060, backgroundColor: 'var(--bg-card-solid)' }}
        ...
        <div>
          <div className="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom border-gold">
            <div className="d-flex align-items-center gap-2">
              ...
              <h5 className="font-serif fs-4 mb-0 text-heading">Shopping Bag</h5>
            </div>
          ...
          {cart.length === 0 ? (
            <div className="text-center py-5 text-muted">
              ...
              <p className="fs-5">Your shopping bag is empty.</p>
            </div>
          ) : (
              ...
              {cart.map(({ product, quantity }) => (
                <div key={product.id} className="d-flex gap-3 mb-3 p-2 border-bottom border-gold align-items-center">
                  ...
                  <div className="flex-grow-1">
                    <h6 className="font-serif mb-1 text-heading">{product.name}</h6>
                    <div className="text-gold fw-bold">${product.price}</div>
        ...
        {cart.length > 0 && (
          <div className="pt-3 border-top border-gold">
            <div className="d-flex justify-content-between fs-5 font-serif mb-3">
              <span>Subtotal:</span>
              <span className="text-gold fw-bold">${totalCartPrice}</span>
            </div>
            <button
              className="btn btn-gold w-100 py-3 d-flex justify-content-center align-items-center gap-2"
              onClick={() => alert('Proceeding to Luxury Checkout!')}
            >
